...
class OscilloGraphe(Canvas):
    ...
    def __init__(self, boss=None, larg=200, haut=150):
        ...
        pas = (larg-25)/8.  #intervalles de l'échelle horizontale. '.' pour avoir un nombre float
        for t in range(1, 9):
            stx=10+t*pas  #+10 pour partir de l'Origine
            self.create_line(stx, haut/2-4, stx, haut/2+4)#Création d'un trait de 4 au dessus de la ligne et 2 en dessous
    def traceCourbe(self, freq=1, phase=0, ampl=10, coul='red'):
        "Tracé d'un graphique élongation/temps sur 1 seconde"
        curve=[] #liste des coordonées
        pas=(self.larg-25)/1000. #l'échelle X correspond à 1 seconde
        for t in range(0,1001, 5):#que l'on divise en 1000ms.
            e=ampl*sin(2*pi*freq*t/1000 - phase)
            x=10+t*pas
            y=self.haut/2-e*self.haut/25
            curve.append((x,y))
        n=self.create_line(curve, fill=coul, smooth=1) #n=numéro d'ordre du tracé

### Code pour tester la class:3##
